...
describe('URL Bookmark Integration Test', () => {
  ...
  it('should allow accessing organization data via bookmarked URL across sessions', () => {
    ...
    // データを追加
    const group1 = createGroup(org.id, {
      name: 'ボーカル',
      order: 1,
      color: '#FF0000',
    });

    const group2 = createGroup(org.id, {
      name: 'ギター',
      order: 2,
      color: '#00FF00',
    });
    ...
    const retrievedOrg = getOrganizationById(bookmarkedOrgId);
    ...
    // データが保持されていることを確認
    const groups = getAllGroups(bookmarkedOrgId);
    expect(groups).toHaveLength(2);
    expect(groups[0].name).toBe('ボーカル');
    expect(groups[1].name).toBe('ギター');

    const members = getAllMembers(bookmarkedOrgId);
    expect(members).toHaveLength(2);
    expect(members[0].name).toBe('田中太郎');
    expect(members[1].name).toBe('鈴木花子');
  });

  it('should handle invalid bookmarked URLs gracefully', () => {
    // 存在しない団体IDでアクセスを試みる
    const invalidOrgId = 'non-existent-organization-id';

    const org = getOrganizationById(invalidOrgId);

    // nullが返されることを確認（404ページに遷移する想定）
    expect(org).toBeNull();
  });

  it('should persist multiple organizations independently via their URLs', () => {
    // 複数の団体を作成
    ...
    // 各団体にデータを追加
    createGroup(org1.id, { name: 'グループA1', order: 1, color: '#FF0000' });
    createGroup(org2.id, { name: 'グループB1', order: 1, color: '#0000FF' });

    // セッションをまたいでアクセス
    const retrievedOrg1 = getOrganizationById(bookmark1);
    const retrievedOrg2 = getOrganizationById(bookmark2);
    ...
    // データが独立していることを確認
    const groupsA = getAllGroups(bookmark1);
    const groupsB = getAllGroups(bookmark2);
