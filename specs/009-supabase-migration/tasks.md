...
- [ ] T058 [Green] [US1] エラーハンドリングを実装（Supabaseエラーを日本語メッセージに変換）
- [ ] T059 [Refactor] [US1] テスト実行・確認、コード改善

**Cycle 13: カスケード削除の検証**
- [ ] T060 [Red] [US1] カスケード削除のテストを作成（organizationを削除して関連データも削除）
- [ ] T061 [Green] [US1] カスケード削除の動作確認（DB側の機能なので実装不要、テストのみ）
- [ ] T062 [Refactor] [US1] テスト実行・確認

**Cycle 14: 一意制約の検証**
- [ ] T063 [Red] [US1] 一意制約のテストを作成（同じevent_date_id + member_idで2回挿入）
- [ ] T064 [Green] [US1] 一意制約の動作確認（DB側の機能なので実装不要、テストのみ）
- [ ] T065 [Refactor] [US1] テスト実行・確認

**Cycle 15: パフォーマンステスト**
- [ ] T066 [Red] [US1] パフォーマンステストを作成（1000+レコードのクエリが500ms以内）
- [ ] T067 [Green] [US1] パフォーマンステスト実行・調整
- [ ] T068 [Refactor] [US1] 全テスト実行、100% pass確認
